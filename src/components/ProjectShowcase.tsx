...
export function ProjectShowcase() {
    ...
    return <section className="showcase-section page-shell" aria-labelledby="projects-title"><div className="section-heading showcase-heading"><div><h2 id="projects-title">Signature Projects</h2></div></div>
        <div className="showcase-interactive" onMouseLeave={() => setActiveId(null)}>
            <div className="project-tabs-mask">
                <div className="project-tabs" role="tablist" aria-label="Signature projects">
...
function ProjectTab({ project, active, onSelect, hidden }: { project: Project; active: boolean; onSelect: (id: string) => void; hidden?: boolean }) {
    return <button className={`project-tab${active ? " is-active" : ""}`} type="button" role="tab" aria-selected={active} aria-hidden={hidden} tabIndex={hidden ? -1 : 0} onClick={() => onSelect(project.id)} onMouseEnter={() => onSelect(project.id)} onFocus={() => onSelect(project.id)}><Image src={project.logo} alt={`${project.name} project logo placeholder`} width={160} height={48} /><span>{project.name}</span></button>;
}